...
- 用户点击“添加当前标签页”时立即捕获当时的可见区域，并把该截图保存为新书签封面。
...
- 排序使用紧贴按钮展开的不透明暗色菜单；列表多选框位于独立左侧列，图标和标题随之右移。
...
1. 用户在目标网页点击“添加当前标签页”。
2. 扩展立即读取页面基础元数据并捕获当前可见区域。
3. 收藏确认框显示截图预览；用户保存后中心裁切为 16:10 WebP 并写入 IndexedDB。
4. 普通点击任何书签都只执行导航与最近使用时间记录，不触发截图。
